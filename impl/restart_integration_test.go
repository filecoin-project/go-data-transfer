	...
	tcs := map[string]struct {
		...
		"Restart peer create push": {
			...
			openPushF: func(rh *restartHarness) datatransfer.ChannelID {
				...
				chid, err := rh.dt1.OpenPushDataChannel(rh.testCtx, rh.peer2, testutil.FakeDTVoucherType, voucher, rh.rootCid, rh.gsData.AllSelector)
			...
			restartF: func(rh *restartHarness, chId datatransfer.ChannelID, subscriber datatransfer.Subscriber) {
				...
				require.NoError(rh.t, err)
				require.NoError(rh.t, rh.dt1.RegisterVoucherType(testutil.FakeDTVoucherType, rh.sv))
		...
		"Restart peer receive push": {
			...
			restartF: func(rh *restartHarness, chId datatransfer.ChannelID, subscriber datatransfer.Subscriber) {
				...
				require.NoError(rh.t, err)
				require.NoError(rh.t, rh.dt2.RegisterVoucherType(testutil.FakeDTVoucherType, rh.sv))
...
func TestRestartPull(t *testing.T) {
	tcs := map[string]struct {
		...
		"Restart peer create pull": {
			...
			openPullF: func(rh *restartHarness) datatransfer.ChannelID {
				voucher := testutil.NewFakeDTTypeNodeWithData("applesauce")
				chid, err := rh.dt2.OpenPullDataChannel(rh.testCtx, rh.peer1, testutil.FakeDTVoucherType, voucher, rh.rootCid, rh.gsData.AllSelector)
				require.NoError(rh.t, err)
				return chid
			},
			restartF: func(rh *restartHarness, chId datatransfer.ChannelID, subscriber datatransfer.Subscriber) {

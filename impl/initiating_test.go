...
func TestDataTransferInitiating(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"SendVoucher with no channel open": {
			verify: func(t *testing.T, h *harness) {
				err := h.dt.SendVoucher(h.ctx, datatransfer.ChannelID{Initiator: h.peers[1], Responder: h.peers[0], ID: 999999}, h.voucherType, h.voucher)
		...
		"SendVoucher with channel open, push succeeds": {
			...
			verify: func(t *testing.T, h *harness) {
				...
				err = h.dt.SendVoucher(ctx, channelID, testutil.FakeDTVoucherType, voucher)
				require.NoError(t, err)
		...
		"reregister voucher type again errors": {
			verify: func(t *testing.T, h *harness) {
				...
				err := h.dt.RegisterVoucherType(h.voucherType, sv)
				require.NoError(t, err)
				err = h.dt.RegisterVoucherType(testutil.FakeDTVoucherType, sv)
		...
		"success response, w/ voucher result": {
			...
			verify: func(t *testing.T, h *harness) {
				...
				require.NoError(t, err)
				require.NotEmpty(t, channelID)
				response, err := message.NewResponse(channelID.ID, true, false, h.voucherResultType, h.voucherResult)
				require.NoError(t, err)
				err = h.transport.EventHandler.OnResponseReceived(channelID, response)
				require.NoError(t, err)
			},
...
func TestDataTransferRestartInitiating(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"RestartDataTransferChannel: Manager Peer Receive Push Restart works ": {
			...
			verify: func(t *testing.T, h *harness) {
				...
				require.True(t, ipld.DeepEqual(h.voucher.(schema.TypedNode).Representation(), vmsg.Voucher))
	...
	for testCase, verify := range testCases {
		t.Run(testCase, func(t *testing.T) {
			...
			require.NoError(t, err)
			h.voucher = datatransfer.Voucher(node)
			h.voucherType = testutil.FakeDTVoucherType
			require.NoError(t, h.dt.RegisterVoucherType(h.voucherType, h.voucherValidator))
			node, err = ipldutil.ToNode(testutil.NewFakeDTType())
			require.NoError(t, err)
...
type harness struct {
	...
	voucherResult     datatransfer.VoucherResult
